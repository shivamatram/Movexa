...
/**
 * Placeholder container fragment for Mechanic role.
 * Displays welcome message and sign-out functionality.
 * Will be replaced with full mechanic dashboard in future implementation.
 */
class MechanicMainContainerFragment : BaseFragment<FragmentMechanicContainerBinding>(
    ...
    override fun initViews() {
        showToolbar(getString(R.string.mechanic_dashboard_title))
        hideBottomNav()

        val session = SessionManager.getInstance()
        val user = session.currentUser.value
        val email = user?.email ?: "Unknown"
        val name = user?.fullName ?: "Mechanic"
