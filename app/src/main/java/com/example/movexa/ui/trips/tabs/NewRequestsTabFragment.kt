...
/**
 * Tab fragment showing new trip requests for the driver (status = ASSIGNED).
 *
 * Hosted inside DriverTripsFragment.
 * Shares the parent fragment's DriverTripsViewModel.
 */
class NewRequestsTabFragment : BaseFragment<FragmentTripTabBinding>(
    ...
    /** Accept trip callback. */
    var onAcceptClick: ((Trip) -> Unit)? = null

    /** Reject trip callback. */
    var onRejectClick: ((Trip) -> Unit)? = null
    ...
    override fun initViews() {
        ...
        binding.tvEmptyTitle.text = getString(R.string.trip_empty_new_requests)
        binding.tvEmptySubtitle.text = getString(R.string.trip_empty_new_requests_subtitle)
    }
    ...
    override fun setupListeners() {
        ...
        adapter.onPrimaryClick = { trip ->
            onAcceptClick?.invoke(trip)
        }

        // Secondary → Reject
        adapter.onSecondaryClick = { trip ->
            onRejectClick?.invoke(trip)
        }
    ...
    override fun observeData() {
        collectLatestFlow(viewModel.newRequests) { state ->
            when (state) {
    ...
    companion object {
        fun newInstance(): NewRequestsTabFragment = NewRequestsTabFragment()
    }
}
